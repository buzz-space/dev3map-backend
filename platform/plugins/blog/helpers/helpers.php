...
function getDataFromCR4($url, $body_request = [])
{
//    /*
    $queryString = [];
    ...
    curl_setopt($ch, CURLOPT_URL, env("CR4_API_URL") . $url . "?" . $queryString);
...
//    $result = json_decode(file_get_contents(public_path("storage/response.json")));
    if (!empty($result))
        return $result->data;
    return [];
}

function getCategoryType($index = false){
    if ($index)
    return [
      Category::TYPE_NEWBIE => ucfirst(Category::TYPE_NEWBIE),
      Category::TYPE_TRADER => ucfirst(Category::TYPE_TRADER),
      Category::TYPE_CREATOR => ucfirst(Category::TYPE_CREATOR),
    ];

    return [Category::TYPE_NEWBIE,Category::TYPE_TRADER,Category::TYPE_CREATOR];
}

function makeFirstImageToAvatar($post){
    $html = $post->bodyFull;
    ...
    if (!empty($imageTags))
        $post->image = $imageTags[0]->getAttribute("src");
    return $post;
}
