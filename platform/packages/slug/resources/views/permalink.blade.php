@php
    $prefix = env("FE_URL") . "/post";
    $value = $value ?: old('slug');
    $endingURL = config('core.base.general.public_single_ending_url');
    //$previewURL = $prefix . '/' . $endingURL . (Auth::user() && $preview ? '?preview=true' : '');
    $previewURL = str_replace('--slug--', $value, $prefix . '/' . config('packages.slug.general.pattern')) . $endingURL . (Auth::user() && $preview ? '?preview=true' : '');
@endphp
